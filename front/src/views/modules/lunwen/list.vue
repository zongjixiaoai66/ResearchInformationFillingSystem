        ...
        <div v-if="showFlag">
            <el-form :inline="true" :model="searchForm" class="form-content">
                <el-row :gutter="20" class="slt" :style="{justifyContent:contents.searchBoxPosition=='1'?'flex-start':contents.searchBoxPosition=='2'?'center':'flex-end'}">
                     ...
                     <el-form-item :label="contents.inputTitle == 1 ? '教研论文名称' : ''">
                         <el-input prefix-icon="el-icon-search" v-model="searchForm.lunwenName" placeholder="教研论文名称" clearable></el-input>
                     </el-form-item>
                         
                     <el-form-item :label="contents.inputTitle == 1 ? '教研论文类型' : ''">
                        <el-select v-model="searchForm.lunwenTypes" placeholder="请选择教研论文类型">
                            <el-option label="=-请选择-=" value=""></el-option>
                            <el-option
                               v-for="(item,index) in lunwenTypesSelectSearch"
                               v-bind:key="index"
                    ...
                    <el-form-item :label="contents.inputTitle == 1 ? '教研项目名称' : ''">
                        <el-input prefix-icon="el-icon-search" v-model="searchForm.xiangmuName" placeholder="教研项目名称" clearable></el-input>
                    </el-form-item>
                                                             
                    <el-form-item :label="contents.inputTitle == 1 ? '教研项目类型' : ''">
                        <el-select v-model="searchForm.xiangmuTypes" placeholder="请选择教研项目类型">
                            <el-option label="=-请选择-=" value=""></el-option>
                            <el-option
                ...
                <el-row class="ad" :style="{justifyContent:contents.btnAdAllBoxPosition=='1'?'flex-start':contents.btnAdAllBoxPosition=='2'?'center':'flex-end'}">
                    <el-form-item>
                        ...
                        <a style="text-decoration:none" class="el-button el-button--success"
                           v-if="isAuth('lunwen','导入导出')"
                           icon="el-icon-download"
                           href="http://localhost:8080/jiaoyanguanlixitong/upload/lunwenMuBan.xls"
                        >批量导入教研论文数据模板</a>
                        &nbsp;
                        <el-upload
                                v-if="isAuth('lunwen','导入导出')"
                                ...
                                :action="getActionUrl"
                                :on-success="lunwenUploadSuccess"
                                :on-error="lunwenUploadError"
                                :show-file-list = false>
                            <el-button
                                    v-if="isAuth('lunwen','导入导出')"
                                    type="success"
                                    icon="el-icon-upload2"
                            >批量导入教研论文数据</el-button>
                        </el-upload>
                        &nbsp;
                        <!-- 导出excel -->
                        <download-excel v-if="isAuth('lunwen','导入导出')" style="display: inline-block" class = "export-excel-wrapper" :data = "dataList" :fields = "json_fields" name = "lunwen.xls">
                            <!-- 导出excel -->
                            <el-button
                                    type="success"
                                    icon="el-icon-download"
            ...
            <div class="table-content">
                <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
                    ...
                    <el-table-column :sortable="contents.tableSortable" :align="contents.tableAlign" prop="xiangmuPhoto"
                        ...
                        <template slot-scope="scope">
                            <div v-if="scope.row.xiangmuPhoto">
                                <img :src="$base.url+scope.row.xiangmuPhoto" width="100" height="100">
                            </div>
                            <div v-else>无图片</div>
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                   ...
                                   label="教研论文名称">
                        <template slot-scope="scope">
                            {{scope.row.lunwenName}}
                        </template>
                    </el-table-column>

                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                   prop="lunwenUuidNumber"
                                   header-align="center"
                                   label="教研论文编号">
                        <template slot-scope="scope">
                            {{scope.row.lunwenUuidNumber}}
                        </template>
                    </el-table-column>
                    <el-table-column :sortable="contents.tableSortable" :align="contents.tableAlign" prop="lunwenPhoto"
                                     header-align="center"
                                     width="200"
                                     label="教研论文照片">
                        <template slot-scope="scope">
                            <div v-if="scope.row.lunwenPhoto">
                                <img :src="$base.url+scope.row.lunwenPhoto" width="100" height="100">
                            </div>
                            <div v-else>无图片</div>
                        </template>
                    </el-table-column>
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                      ...
                                      label="教研论文类型">
                        <template slot-scope="scope">
                            {{scope.row.lunwenValue}}
                        </template>
                    </el-table-column>
                    <el-table-column :sortable="contents.tableSortable" :align="contents.tableAlign" prop="lunwenFile"
                                     header-align="center"
                                     width="200"
                                     label="教研论文下载">
                        <template slot-scope="scope">
                            <div v-if="scope.row.lunwenFile">
                                <a type="text" style="text-decoration:none" class="el-button" :href="$base.url+scope.row.lunwenFile">下载</a>
                            </div>
                            <div v-else>无文件</div>
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column width="300" :align="contents.tableAlign"
                        ...
                        <template slot-scope="scope">
                            ...
                            <el-button v-if="isAuth('lunwen','查看')" type="success" icon="el-icon-tickets" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">详情</el-button>
                            <el-button v-if="isAuth('lunwen','修改')" type="primary" icon="el-icon-edit" size="mini" @click="addOrUpdateHandler(scope.row.id)">修改</el-button>
                            <el-button v-if="isAuth('lunwen','删除')" type="danger" icon="el-icon-delete" size="mini" @click="deleteHandler(scope.row.id)">删除</el-button>
        ...
        methods: {
            chartDialog() {
                ...
                let params = {
                    ...
                    thisTable : {//当前表
                        tableName :'lunwen',//当前表表名,
                        // sumColum : 'lunwen_number', //求和字段
                        date : 'insert_time',//分组日期字段
                        // string : 'lunwen_name',//分组字符串字段
                        types : 'lunwen_types',//分组下拉框字段
                    },
                    // joinTable : {//级联表（可以不存在）
            ...
            getDataList() {
                ...
                if (this.searchForm.lunwenName!= '' && this.searchForm.lunwenName!= undefined) {
                    params['lunwenName'] = '%' + this.searchForm.lunwenName + '%'
                }
                         
                if (this.searchForm.lunwenTypes!= '' && this.searchForm.lunwenTypes!= undefined) {
                    params['lunwenTypes'] = this.searchForm.lunwenTypes
                }
                                        
                params['lunwenDelete'] = 1// 逻辑删除字段 1 未删除 2 删除
                ...
                //填充下拉框选项
                this.$http({
                    url: "dictionary/page?dicCode=lunwen_types&page=1&limit=100",
                    method: "get",
                }).then(({data}) => {
                    if(data && data.code === 0){
                        this.lunwenTypesSelectSearch = data.data.list;
                    }
                });
            },
            //每页数
            ...
            // 导入功能上传文件成功后调用导入方法
            lunwenUploadSuccess(data){
                let _this = this;
                _this.$http({
                    url: "lunwen/batchInsert?fileName=" + data.file,
                    method: "get"
                }).then(({data}) => {
                    if(data && data.code === 0){
                        _this.$message({
